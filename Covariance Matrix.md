...
For simple, uni-variate data (data with only one variable), standard deviation works perfectly. It gives me a single number representative of the average spread of the data.
...
>- Each data point $x_{i}$ is a vector, The mean $\mu$ is also a vector.
>- The term $x_{i} - \mu$ is the **deviation vector**, showing how far the point is from the center in each dimension.
>- And $\| \vec{x_{i}} - \vec{\mu} \|^{2}_{2}$ is the square of the L2 norm of the deviation vector.
>- Average all these and I have my variance. $\rightarrow$
>
> $$\large\sigma = \frac{1}{N}{\sum_{i=1}^{N}\| \vec{x_{i}} - \vec{\mu} \|^{2}_{2}}$$
>
...
If have $n$ data points and each has $d$ dimensions, the data matrix $D$ would be an $n \times d$ matrix:
...
>
>$$C = \frac{1}{N}{\sum_{i=1}^{N} (x^{i} - \mu)(x^{i} - \mu)^{T}}$$
>
>Where $x^{i}$ is a column vector denoting the $i$-th data point.
...
> - This is simply because: $\text{cov}(x^{j},x^{k})= \text{cov}(x^{k},x^{j})$

###### Deriving the generalized notation for the Covariance matrix

$x^{i}$ is a column vector representing a single data point. And $\mu$ is the mean vector.
...
For each data-point I calculate the deviation matrix, for $x^{1}$ I have $M_{1}$, for $x^{2}$ I have $M_{2}$ and so on.
To get the covariance matrix I can just add up all the deviation matrices and then finally divide by $N$.

$$\begin{align} C & =\frac{1}{N}\Big[M_{1} + M_{2} + \cdots + M_{N}\Big] \\ \\ & =  \frac{1}{N}\Big[(x^{1} - \mu)(x^{1} - \mu)^{T} + (x^{2} - \mu)(x^{2} - \mu)^{T} + \cdots + (x^{N} - \mu)(x^{N} - \mu)^{T}\Big] \\ \\ & =  \frac{1}{N}\sum_{i=1}^{N}(x^{i} - \mu)(x^{i} - \mu)^{T} \end{align} $$
